                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex flex-wrap items-center gap-4">
                                ...
                                <div class="flex flex-wrap items-center gap-3">
                                    ...
                                    @if ($filterType !== 'all')
                                        <x-filament::button color="danger" size="sm" tooltip="Remove Filters"
                                            wire:click="resetFilters" class="self-center" icon="heroicon-o-x-mark">
                                        </x-filament::button>
                                    @endif
                                </div>
                            </div>
                            ...
                            {{-- Create New asset --}}
                            <x-filament::icon-button href="{{ route('filament.app.resources.assets.create') }}"
                            ...
                            <div>
                                ...
                                <div x-data="{ showImportModal: false, importSuccess: false }" class="relative">
                                    <!-- Import Button -->
                                    <x-filament::icon-button tooltip="Import Excel" @click="showImportModal = true"
                            ...
                            {{-- Filter Status Available or Deployed Assets --}}
                            <div class="flex items-center gap-2">
                                <x-filament::icon-button
                                    wire:click="filterByStatus('{{ $statusFilter === 'available' ? 'deployed' : 'available' }}')"
                                    :color="$statusFilter ? 'primary' : 'secondary'" icon="heroicon-m-arrow-path" :tooltip="$statusFilter === 'available' ? 'View Deployed Assets' : ($statusFilter === 'deployed' ? 'View Available Assets' : 'View Available Assets')" size="md" />

                                @if ($statusFilter)
                                    <x-filament::icon-button wire:click="filterByStatus('')" color="danger"
                                        icon="heroicon-m-x-mark" tooltip="Show All Assets" size="md" />
                                @endif
                            </div>
                        </div>
    </x-filament::section>
    ...
    <section>
        <x-filament::section class="my-4" collapsible>
            ...
            <section class="grid grid-cols-1 md:grid-cols-2 gap-2 my-4">
                ...
                <section class="p-4 border border-gray-600 rounded-lg">
                                        ...
                                        @if ($asset->expiry_date && ($isSoftware || $hasLicenseTag))
                                            <p>
                                                ...
                                                @elseif ($daysUntilExpiry < 30)
                                                    <x-filament::badge color="warning">Expiring
                                                        soon</x-filament::badge>
                                                @endif
                                            </p>
                                        @endif
                                    </div>
                            ...
                            <x-filament-tables::row>
                                <x-filament-tables::header-cell scope="col" class="w-12 px-4 py-3">
                                    <x-filament::input.checkbox wire:model.live="selectAll"
                                        @click="toggleAllCheckboxes()" id="select-all-table" class="rounded" />
                            ...
                            @forelse($assets as $asset)
                                <x-filament-tables::row :class="in_array((string) $asset->id, $selected) ? 'bg-primary-50' : ''">
                                    <x-filament-tables::cell class="w-12 px-4 py-4 whitespace-nowrap">
                                        <x-filament::input.checkbox wire:model.live="selected"
                                            value="{{ $asset->id }}"
                                            wire:key="table-checkbox-{{ $asset->id }}" :class="in_array((string) $asset->id, $selected) ? 'bg-primary-500' : ''" />
